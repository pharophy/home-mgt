...
The system SHALL present shared-device tablet surfaces with touch-sized controls, safe-area-aware spacing, and reachable primary actions suited for common household tablet use.
...
### Requirement: Tablet workflows SHALL keep primary actions reachable while content scrolls
The system SHALL keep the main action for the current tablet workflow accessible without requiring the user to repeatedly scroll to the far top or bottom of a long workspace.
...
#### Scenario: Child uses the active task surface
- **WHEN** the child-facing tablet task surface is visible
- **THEN** the single next-step completion action remains easy to reach in the active viewport
- **THEN** the child does not need fine-grained scrolling to find the main action
