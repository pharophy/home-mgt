...
function normalizeConnectionString(connectionString: string): string {
  const normalizedParts = connectionString
    ...
    .map((part) => {
      ...
      if (
        normalizedKey === "trusted_connection" ||
        normalizedKey === "trustservercertificate"
      ) {
        return null;
      }
...
export class MssqlParticipationClient implements SqlParticipationClient {
  ...
  async ensureSchema(): Promise<void> {
    ...
    await applySqlMigrations(new MssqlSqlMigrationExecutor(() => this.getPool()));
